...
`luaconfig` is centered on automation work in Lua. The technical target is: Resolve layered config values with includes and provenance.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 3.
